...
[z,xbig] = Paper_MakeData_52(T,S,A,G,Bbig,Cbig,p);
...
%     sum(abs(Q(:)-QQ(:)))
    ...
    [zDKF(:,:,k,1),~,~,pdFlag] = Paper_DKF_Block(x,S,A,G,f,Q,false);
    ...
    zDKF(:,:,k,2) = Paper_DKF_Block(x,S,A,G,f,Q,true);
    ...
    zADF(:,:,k) = Paper_GADFilter_52(x,A,S,G,p,B,C);
    ...
    for j = 1:mN
        ...
        zPF(:,:,k,j) = Paper_ParticleFilter_52(m,x,S,A,G,B,C,p,true);
...
pl7 = legend([pl3,pl6,pl5,pl4,pl2,pl1],{'G-ADF','DKF','robust DKF','f(X_t)','particle filter','KF, EKF, UKF'}, 'Location','southwest');
...
pl72 = legend([pl32,pl62,pl52,pl42,pl22],{'G-ADF','DKF','robust DKF','f(X_t)','particle filter'}, 'Location','northwest');
