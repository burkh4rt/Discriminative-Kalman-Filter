...
%   Y = LOGMVNPDF(X) returns the log probability density of the multivariate normal
...
%   N-by-1 vector.
%
%   Y = LOGMVNPDF(X,MU) returns the log density of the multivariate normal
...
%   row of MU.  MU can also be a scalar value, which LOGMVNPDF replicates to
%   match the size of X.
%
%   Y = LOGMVNPDF(X,MU,SIGMA) returns the log density of the multivariate normal
...
%   of SIGMA, i.e., LOGMVNPDF computes Y(I) using X(I,:) and SIGMA(:,:,I).
...
%
%   See also MNVPDF, MVTPDF, MVNCDF, MVNRND, NORMPDF.
%
% Modified from Matlab's mvnpdf.m by Matthew Harrison 9-22-2017.
% The only algorithmic change was to remove exponentiation of the output Y.

%   Copyright 1993-2015 The MathWorks, Inc.


if nargin<1
    error(('logmvnpdf:TooFewInputs'));
elseif ndims(X)~=2
    error(('logmvnpdf:InvalidData'));
end

% Get size of data.  Column vectors provisionally interpreted as multiple scalar data.
[n,d] = size(X);
if d<1
    error(('logmvnpdf:TooFewDimensions'));
end
...
elseif ndims(Mu) == 2
    ...
    if d2 ~= d % has to have same number of coords as X
        error(('logmvnpdf:ColSizeMismatch'));
    ...
    elseif n == 1 % data is a single row, rep it out to match mean
        ...
        error(('logmvnpdf:RowSizeMismatch'));
    end
    
else
    error(('logmvnpdf:BadMu'));
end
...
elseif ndims(Sigma) == 2
    ...
    if sz(1) ~= sz(2)
        error(('logmvnpdf:BadCovariance'));
    elseif ~isequal(sz, [d d])
        error(('logmvnpdf:CovSizeMismatch'));
    else
        if sigmaIsDiag
            if any(Sigma<=0)
                error(('logmvnpdf:BadDiagSigma'));
            end
            R = sqrt(Sigma);
            ...
            if err ~= 0
                error(('logmvnpdf:BadMatrixSigma'));
            end
...
elseif ndims(Sigma) == 3
    ...
    if sz(1) ~= sz(2)
        error(('logmvnpdf:BadCovarianceMultiple'));
    elseif (sz(1) ~= d) || (sz(2) ~= d) % Sigma is a stack of dxd matrices
        error(('logmvnpdf:CovSizeMismatchMultiple'));
    elseif sz(3) ~= n
        error(('logmvnpdf:CovSizeMismatchPages'));
    else
        if sigmaIsDiag
            if any(any(Sigma<=0))
                error(('logmvnpdf:BadDiagSigma'));
            ...
            for i = 1:n
                ...
                if err ~= 0
                    error(('logmvnpdf:BadMatrixSigmaMultiple'));
                end
                xRinv(i,:) = X0(i,:) / R;
